...
class TestQuestionAPIAnswer:
    def create_test_question(self, student_id: str = None) -> str:
        if student_id is None: student_id = str(uuid4())
        question_data = {"title": "Вопрос для ответа","content": "Содержание вопроса", "student_id": student_id}
    ...
    def test_answer_question_success(self, start_server: None):
        ...
        response = requests.post(f"{BASE_URL}/{question_id}/answer",params={"teacher_id": teacher_id,"answer_text": answer_text})
...
class TestQuestionAPIClose:
    def create_answered_question(self) -> str:
        ...
        question_data = { "title": "Вопрос для закрытия","content": "Этот вопрос будет закрыт","student_id": student_id }
        response = requests.post(BASE_URL, json=question_data)
        question_id = response.json()["id"]
        teacher_id = str(uuid4())
        requests.post(f"{BASE_URL}/{question_id}/answer",params={"teacher_id": teacher_id,"answer_text": "Ответ перед закрытием"})
        return question_id
